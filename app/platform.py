...
class TradingPlatform:
    ...
    def scan_signals(self, max_signals: int = 40, enabled_feeds: Optional[List[str]] = None) -> int:
        ...
        self.state["signals"] = [s.to_dict() for s in signals]
        ...
        self.state["journal"].append({"ts": now_iso(), "event": "shark_scan_completed", "mode": "live_only", "signals": len(signals), "alerts": len(alerts), "top_score": alerts[0].shark_score if alerts else 0, "feed_status": reliability.system_status})
        self.save()
        return len(signals)
    ...
    def auto_paper_top_alert(self, alert_id: str, dollars: float = 800) -> Dict[str, Any]:
        ...
        qty = min(requested_qty, max(1, int(sizing["quantity"])))
        side = "buy" if inst.get("direction", alert.direction).upper() in ["BUY", "LONG"] else "sell"
        return self.place_order({"symbol": inst["symbol"], "asset_type": inst.get("asset_type", "stock"), "side": side, "quantity": qty, "order_type": "market", "mark_price": q["last"], "spread_bps": q.get("spread_bps", 0), "notes": f"Auto paper from shark alert {alert.narrative}; sizing={sizing}", "alert_id": alert.id})
